...
以下以**在安装 Photoshop 的 Windows 上，用同一用户打开本仓库**为例。UDT 会话必须对应本机的当前工具包；云端任务或另一台电脑的终端不能直接连接这套本地工具。工具根是仓库下的 psd2ui 目录，不是 Unity 项目目录。

## 1. 安装面板并准备工具依赖

先按 [仓库 README](../README.md) 安装 **UXP / CCX 0.2.0** 并打开面板。安装 Node.js（20.x 至少 20.19，22.x 至少 22.12，或工具支持的更新版本）。本分支还需要 PowerShell 7、UDT 与 Photoshop 的 UXP 开发模式。
...
InstallDependencies 安装锁定依赖；Connect 准备 UDT 与本包插件会话，Check 检查已有连接。Check 成功后再配置客户端。日常任务只需 Check，不用每次重新安装。
...
以上示例文件不会自动修改你的客户端设置。PSD2UI 使用 **stdio**：客户端启动 node 进程即可，不需要填写 localhost URL或 API Key。CLI 单独启动 server.js 后等待输入属于正常现象。
...
> 继续上一份已确认方案，沿用相同 PSD 和交付目录。只做尚未完成的步骤；如果上次写操作超时，先核对连接和当前 PSD 的实际状态，不重复提交。
...
| 任意移动、解组、删除 | 此 CCX 分支没有这些结构扩展，由人工操作承接 |
...
| 工具可见，但 Photoshop 状态失败 | 完成 UDT/插件的显式 Connect，确认加载的是本分支源码，再严格 Check |
...
| 写操作超时或断连 | 先核对 PSD 与当前连接，不把超时当作已经取消并自动重试 |
