...
for (const kind of ['containers', 'groups']) test(`AI ${kind} rejects interleaved members instead of crossing an unselected layer`, async () => {
  const items = [layer(2, 'A'), layer(3, 'B'), layer(4, 'C')];
  const host = photoshopHost(items);
  const plan = { version: 1, confirmationId: 'interleaved-members',
    preconditions: items.map(item => ({ layerId: String(item.id), parentId: '1' })),
    [kind]: [{ alias: '@combined', name: '组', members: ['2', '4'], semantic: 'button', roles: [] }] };
  await assert.rejects(host.api.applyConfirmedStructurePlan(plan, async () => {}), /夹有未选图层/);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 3, 4]);
});

test('grouping cannot separate a clipped image from its base', async () => {
  const image = layer(2, '剪贴纹理'); image.clipped = true;
  const base = layer(3, '基底');
  const host = photoshopHost([image, base]);
  const plan = { version: 1, confirmationId: 'clipping-boundary',
    preconditions: [{ layerId: '2' }], containers: [{ alias: '@state', name: '状态', members: ['2'] }] };
  await assert.rejects(host.api.applyConfirmedStructurePlan(plan, async () => {}), /剪贴蒙版链/);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 3]);
});

test('new structural edits require explicit appearance intent and exact descendant scope', async () => {
  const item = layer(2, '保留');
  const host = photoshopHost([item]);
  const plan = { version: 1, confirmationId: 'delete-scope', preconditions: [{ layerId: '2' }], deletes: [{ ref: '2' }] };
  await assert.rejects(host.api.applyConfirmedStructurePlan(plan, async () => {}), /allowAppearanceChange/);
  plan.deletes[0].allowAppearanceChange = true;
  await assert.rejects(host.api.applyConfirmedStructurePlan(plan, async () => {}), /expectedDescendantIds/);
  assert.deepEqual(host.document.layers.map(item => item.id), [2]);
});
...
test('confirmed plans accept the virtual top-level parent returned by a full-document snapshot', async () => {
  const group = layer(2, '确认按钮', 'group'); group.layers = [layer(3, 'comm_sp_0001')];
  const panel = await panelHarness([group], { stored: sourceManifest([group]) });
  const root = panel.host.api.createSnapshot('document-root').root;
  const item = root.children[0];
  assert.equal(item.parentId, 'document-root');
  const plan = { version: 1, confirmationId: 'snapshot-parent',
    preconditions: [item, item.children[0]].map(n => ({ layerId: n.layerId, name: n.name, parentId: n.parentId, bounds: n.bounds })),
    adopt: [{ ref: '2', semantic: 'button', roles: [{ name: 'background', ref: '3' }] }] };
  ...
  assert.equal(result.structured[0].semantic, 'button');
  assert.equal(panel.stored.nodes['2'].structure.roles[0].layerId, '3');
});

test('virtual document-root parent cannot authorize a layer currently inside another group', async () => {
  const group = layer(2, '父组', 'group'); group.layers = [layer(3, 'comm_sp_0001')];
  const host = photoshopHost([group]);
  let persisted = false;
  const plan = { version: 1, confirmationId: 'wrong-virtual-parent',
    preconditions: [{ layerId: '3', parentId: 'document-root' }], presets: [{ ref: '3', semantic: 'image' }] };
  await assert.rejects(host.api.applyConfirmedStructurePlan(plan, async () => { persisted = true; }), /父级前置条件不符/);
  assert.equal(persisted, false);
  assert.equal(host.modalCount, 0);
});

test('point text preconditions use snapshot geometry without effects and still reject an actual move', async () => {
  const label = layer(3, '确认', 'text', { left: 5, top: 5, right: 105, bottom: 45 });
  label.boundsNoEffects = { left: 10, top: 10, right: 100, bottom: 40 };
  label.textItem.isPointText = true;
  const group = layer(2, '按钮', 'group'); group.layers = [label, layer(4, 'comm_bt_0001')];
  const host = photoshopHost([group]);
  const item = host.api.createSnapshot('document-root').root.children[0].children[0];
  assert.equal(item.bounds.left, 10);
  const plan = { version: 1, confirmationId: 'point-text-geometry',
    preconditions: [{ layerId: item.layerId, parentId: item.parentId, bounds: item.bounds }],
    presets: [{ ref: '3', semantic: 'text' }] };
  let writes = 0;
  await host.api.applyConfirmedStructurePlan(plan, async () => { writes++; });
  assert.equal(writes, 1);
  label.bounds = { left: 15, top: 5, right: 115, bottom: 45 };
  label.boundsNoEffects = { left: 20, top: 10, right: 110, bottom: 40 };
  await assert.rejects(host.api.applyConfirmedStructurePlan(plan, async () => { writes++; }), /left 前置条件不符/);
  assert.equal(writes, 1);
});
...
test('CEP selection refresh reuses document projection but invalidates it for edits and explicit refresh', async () => {
  let revision = 1, reads = 0, snapshots = 0;
  const h = await panelHarness([layer(2, 'comm_sp_001'), layer(3, 'comm_sp_002')], {
    getDocumentRevision: () => revision, beforeRead: async () => { reads++; }, beforeSnapshot: () => { snapshots++; }
  });
  await h.sandbox.refreshAuthoringState();
  const original = reads;
  const originalSnapshots = snapshots;
  h.host.document.activeLayers = [h.host.document.layers[1]];
  await h.sandbox.refreshAuthoringState({ reuseDocument: true });
  assert.equal(reads, original);
  assert.equal(snapshots, originalSnapshots, 'selection must not rebuild the document snapshot');
  assert.match(h.elements['layer-config-status'].textContent, /自动识别/);
  const documentId = h.host.document.id;
  h.host.document.id = documentId + 1;
  await h.sandbox.refreshAuthoringState({ reuseDocument: true });
  assert.equal(reads, original + 1, 'another document must not reuse the previous manifest');
  h.host.document.id = documentId;
  await h.sandbox.refreshAuthoringState({ reuseDocument: true });
  assert.equal(reads, original + 1, 'returning to an unchanged document reuses its own projection');
  revision++;
  await h.sandbox.refreshAuthoringState({ reuseDocument: true });
  assert.equal(reads, original + 2);
  await h.sandbox.refreshAuthoringState();
  assert.equal(reads, original + 3);
  revision = null;
  await h.sandbox.refreshAuthoringState({ reuseDocument: true });
...
function structuralFixture(options = {}) {
  const nested = layer(10, '嵌套', 'group'); nested.layers = [layer(11, '嵌套图片')];
  const source = layer(3, '来源组', 'group'); source.layers = [layer(4, '移动图片'), layer(5, '原组保留'), nested];
  const target = layer(6, '目标组', 'group'); target.layers = [layer(7, '排序参照'), layer(8, '目标保留')];
  const host = photoshopHost([layer(2, '顶部'), source, target, layer(9, '底部')], options.host || {});
  const operations = [];
  const allLayers = () => {
    const result = [];
    const visit = items => items.forEach(item => { result.push(item); visit(item.layers || []); });
    visit(host.document.layers); return result;
  };
  const ungroup = item => {
    operations.push('ungroup');
    if (options.ungroupNoop) return;
    const parent = item.parent;
    const children = options.ungroupReverse ? item.layers.slice().reverse() : item.layers.slice();
    parent.layers.splice(parent.layers.indexOf(item), 1, ...children);
    children.forEach(child => { child.parent = options.ungroupWrongParent ? item : parent; });
    if (options.ungroupBounds && children.length) children[0].bounds = { ...children[0].bounds, left: children[0].bounds.left + 3 };
    if (options.ungroupLoseNested) nested.layers = [];
  };
  const move = (item, parent, placement) => {
    operations.push('move');
    if (options.moveNoop) return;
    const oldParent = item.parent;
    oldParent.layers.splice(oldParent.layers.indexOf(item), 1);
    const beforeId = placement.beforeId, afterId = placement.afterId;
    let index = beforeId != null ? parent.layers.findIndex(child => child.id === beforeId)
      : afterId != null ? parent.layers.findIndex(child => child.id === afterId) + 1 : 0;
    if (options.moveWrongOrder) index = parent.layers.length;
    parent.layers.splice(index, 0, item);
    item.parent = options.moveWrongParent ? oldParent : parent;
    if (options.moveSourceOrder) oldParent.layers.reverse();
    if (options.moveBounds) item.bounds = { ...item.bounds, left: item.bounds.left + 3 };
    if (options.moveChildBounds && item.layers.length) item.layers[0].bounds = { ...item.layers[0].bounds, left: item.layers[0].bounds.left + 3 };
  };
  host.photoshop.constants.ElementPlacement = { PLACEBEFORE: 'before', PLACEAFTER: 'after', PLACEATBEGINNING: 'beginning' };
  allLayers().forEach(item => {
    if (options.moveFallback) item.move = async (anchor, position) => {
      const parent = position === 'beginning' ? anchor : anchor.parent;
      move(item, parent, { beforeId: position === 'before' ? anchor.id : null, afterId: position === 'after' ? anchor.id : null });
    };
    else item.moveTo = async (parent, placement) => move(item, parent, placement);
    if (!options.ungroupFallback) item.ungroup = async () => ungroup(item);
    item.delete = async () => {
      operations.push('delete');
      if (options.deleteNoop) return;
      if (options.deleteKeepChildren) {
        const parent = item.parent;
        parent.layers.splice(parent.layers.indexOf(item), 1, ...item.layers);
        item.layers.forEach(child => { child.parent = parent; });
      } else item.parent.layers.splice(item.parent.layers.indexOf(item), 1);
      if (options.deleteUnselected) host.document.layers = host.document.layers.filter(child => child.id !== 9);
    };
  });
  if (options.ungroupFallback) {
    const batchPlay = host.photoshop.action.batchPlay;
    host.photoshop.action.batchPlay = async (commands, settings) => {
      await batchPlay(commands, settings);
      if (commands.some(command => command._obj === 'ungroupLayersEvent')) ungroup(host.document.activeLayers[0]);
      return [];
    };
  }
  return { ...host, get modalCount() { return host.modalCount; }, operations, source, target, nested, allLayers };
}

function structuralPlan(host, edits) {
  return { version: 1, confirmationId: 'confirmed-structural-edit',
    preconditions: host.allLayers().map(item => ({ layerId: String(item.id), name: item.name,
      parentId: String(item.parent.id), bounds: { ...item.bounds } })), ...edits };
}

test('protection preconditions compare fields independently of JSON property order and recheck after the modal wait', async () => {
  const item = layer(2, '保护图层'); item.blendMode = 'normal';
  const host = photoshopHost([item]);
  const protection = { blendMode: 'normal', opacity: 100, hasVectorMask: false, hasLayerMask: false, clipped: false };
  const plan = { version: 1, confirmationId: 'same-protection', preconditions: [{ layerId: '2', protection }] };
  await host.api.applyConfirmedStructurePlan(plan, async value => value);
  assert.deepEqual(host.history, [true]);
  const changed = photoshopHost([layer(2, '保护图层')], { beforeModal(document) { document.layers[0].clipped = true; } });
  await assert.rejects(changed.api.applyConfirmedStructurePlan({ ...plan,
    preconditions: [{ layerId: '2', protection: { ...protection, blendMode: '' } }] }, async () => {}), /蒙版、剪贴关系或混合设置已变化/);
  assert.deepEqual(changed.history, []);
});

for (const [label, edit, expectedSource, expectedTarget, expectedRoot] of [
  ['before', { ref: '4', parentRef: '6', beforeRef: '7' }, [5, 10], [4, 7, 8], [2, 3, 6, 9]],
  ['after', { ref: '4', parentRef: '6', afterRef: '7' }, [5, 10], [7, 4, 8], [2, 3, 6, 9]],
  ['same parent', { ref: '4', parentRef: '3', afterRef: '5' }, [5, 4, 10], [7, 8], [2, 3, 6, 9]],
  ['document root', { ref: '4', parentRef: 'document-root', beforeRef: '6' }, [5, 10], [7, 8], [2, 3, 4, 6, 9]],
  ['group subtree', { ref: '10', parentRef: '6', beforeRef: '7' }, [4, 5], [10, 7, 8], [2, 3, 6, 9]]
]) test(`confirmed move preserves exact hierarchy, order and bounds: ${label}`, async () => {
  const host = structuralFixture({ moveFallback: label === 'document root' });
  const plan = structuralPlan(host, { moves: [{ ...edit, allowAppearanceChange: true }] });
  const result = await host.api.applyConfirmedStructurePlan(plan, async value => value);
  assert.deepEqual(host.source.layers.map(item => item.id), expectedSource);
  assert.deepEqual(host.target.layers.map(item => item.id), expectedTarget);
  assert.deepEqual(host.document.layers.map(item => item.id), expectedRoot);
  assert.deepEqual(host.nested.layers.map(item => item.id), [11]);
  assert.equal(result.edits[0].operation, 'move');
  assert.equal(result.edits[0].layerId, edit.ref);
  assert.deepEqual(host.history, [true]);
});

for (const [edit, message] of [
  [{ ref: '3', parentRef: '10' }, /循环引用/],
  [{ ref: '3', parentRef: '3' }, /循环引用/],
  [{ ref: '4', parentRef: '7' }, /循环引用的组/],
  [{ ref: '4', parentRef: '6', beforeRef: '5' }, /排序参照/],
  [{ ref: '4', parentRef: '3', beforeRef: '4' }, /排序参照/],
  [{ ref: '4', parentRef: '6', beforeRef: '7', afterRef: '8' }, /同时指定/]
]) test(`confirmed move rejects invalid destination or anchors: ${JSON.stringify(edit)}`, async () => {
  const host = structuralFixture();
  await assert.rejects(host.api.applyConfirmedStructurePlan(structuralPlan(host,
    { moves: [{ ...edit, allowAppearanceChange: true }] }), async () => {}), message);
  assert.deepEqual(host.operations, []);
  assert.deepEqual(host.source.layers.map(item => item.id), [4, 5, 10]);
});

for (const option of ['moveNoop', 'moveWrongOrder', 'moveWrongParent', 'moveSourceOrder', 'moveBounds', 'moveChildBounds']) {
  test(`confirmed move rolls back a wrong host result before persistence: ${option}`, async () => {
    const host = structuralFixture({ [option]: true });
    const ref = option === 'moveChildBounds' ? '10' : '4';
    let persisted = false;
    await assert.rejects(host.api.applyConfirmedStructurePlan(structuralPlan(host,
      { moves: [{ ref, parentRef: '6', beforeRef: '7', allowAppearanceChange: true }] }), async () => { persisted = true; }), /不符/);
    assert.equal(persisted, false);
    assert.deepEqual(host.source.layers.map(item => item.id), [4, 5, 10]);
    assert.deepEqual(host.target.layers.map(item => item.id), [7, 8]);
    assert.equal(host.nested.layers[0].bounds.left, 0);
    assert.deepEqual(host.history, [false]);
  });
}

for (const fallback of [false, true]) test(`confirmed ungroup promotes only direct children at the original position, fallback=${fallback}`, async () => {
  const host = structuralFixture({ ungroupFallback: fallback });
  const plan = structuralPlan(host, { ungroups: [{ ref: '3', expectedDescendantIds: ['4', '5', '10', '11'], allowAppearanceChange: true }] });
  const result = await host.api.applyConfirmedStructurePlan(plan, async value => value);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 4, 5, 10, 6, 9]);
  assert.deepEqual(host.nested.layers.map(item => item.id), [11]);
  assert.equal(host.nested.parent, host.document);
  assert.equal(host.nested.layers[0].parent, host.nested);
  assert.equal(result.edits[0].operation, 'ungroup');
  assert.deepEqual(host.history, [true]);
  assert.equal(host.photoshop.app.activeDocument, host.document);
  // The UXP fallback must not open another executeAsModal inside the active transaction.
  assert.equal(host.modalCount, 1);
});

for (const option of ['ungroupNoop', 'ungroupReverse', 'ungroupWrongParent', 'ungroupBounds', 'ungroupLoseNested']) {
  test(`confirmed ungroup rolls back missing, reordered or moved contents: ${option}`, async () => {
    const host = structuralFixture({ [option]: true });
    let persisted = false;
    await assert.rejects(host.api.applyConfirmedStructurePlan(structuralPlan(host,
      { ungroups: [{ ref: '3', expectedDescendantIds: ['4', '5', '10', '11'], allowAppearanceChange: true }] }), async () => { persisted = true; }), /不符/);
    assert.equal(persisted, false);
    assert.deepEqual(host.document.layers.map(item => item.id), [2, 3, 6, 9]);
    assert.deepEqual(host.source.layers.map(item => item.id), [4, 5, 10]);
    assert.deepEqual(host.nested.layers.map(item => item.id), [11]);
    assert.deepEqual(host.history, [false]);
  });
...
test('confirmed delete removes exactly the declared subtree and reports its descendants', async () => {
  const host = structuralFixture();
  const result = await host.api.applyConfirmedStructurePlan(structuralPlan(host,
    { deletes: [{ ref: '3', expectedDescendantIds: ['4', '5', '10', '11'], allowAppearanceChange: true }] }), async value => value);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 6, 9]);
  assert.deepEqual(Array.from(result.edits[0].descendantIds), ['4', '5', '10', '11']);
  assert.deepEqual(host.target.layers.map(item => item.id), [7, 8]);
  assert.deepEqual(host.history, [true]);
});

for (const option of ['deleteNoop', 'deleteKeepChildren', 'deleteUnselected']) test(`confirmed delete rejects an incomplete or overbroad host result: ${option}`, async () => {
  const host = structuralFixture({ [option]: true });
  let persisted = false;
  await assert.rejects(host.api.applyConfirmedStructurePlan(structuralPlan(host,
    { deletes: [{ ref: '3', expectedDescendantIds: ['4', '5', '10', '11'], allowAppearanceChange: true }] }), async () => { persisted = true; }), /不符/);
  assert.equal(persisted, false);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 3, 6, 9]);
  assert.deepEqual(host.source.layers.map(item => item.id), [4, 5, 10]);
  assert.deepEqual(host.history, [false]);
...
for (const kind of ['ungroups', 'deletes']) test(`confirmed ${kind} rejects a stale descendant list before the host operation`, async () => {
  const host = structuralFixture();
  await assert.rejects(host.api.applyConfirmedStructurePlan(structuralPlan(host,
    { [kind]: [{ ref: '3', expectedDescendantIds: ['4', '5'], allowAppearanceChange: true }] }), async () => {}), /子层清单已变化/);
  assert.deepEqual(host.operations, []);
  assert.deepEqual(host.history, [false]);
});

test('a failed manifest write rolls back a combined move, ungroup and delete transaction', async () => {
  const host = structuralFixture();
  const plan = structuralPlan(host, {
    moves: [{ ref: '4', parentRef: '6', beforeRef: '7', allowAppearanceChange: true }],
    ungroups: [{ ref: '3', expectedDescendantIds: ['5', '10', '11'], allowAppearanceChange: true }],
    deletes: [{ ref: '10', expectedDescendantIds: ['11'], allowAppearanceChange: true }]
  });
  await assert.rejects(host.api.applyConfirmedStructurePlan(plan, async () => { throw new Error('mirror persistence failed'); }), /mirror persistence failed/);
  assert.deepEqual(host.operations, ['move', 'ungroup', 'delete']);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 3, 6, 9]);
  assert.deepEqual(host.source.layers.map(item => item.id), [4, 5, 10]);
  assert.deepEqual(host.target.layers.map(item => item.id), [7, 8]);
  assert.deepEqual(host.nested.layers.map(item => item.id), [11]);
...
test('document and layer IDs sharing a numeric value do not create a false ancestry cycle', async () => {
  const host = structuralFixture();
  host.document.id = 3;
  await host.api.applyConfirmedStructurePlan(structuralPlan(host,
    { moves: [{ ref: '3', parentRef: '6', beforeRef: '7', allowAppearanceChange: true }] }), async value => value);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 6, 9]);
  assert.deepEqual(host.target.layers.map(item => item.id), [3, 7, 8]);
  assert.deepEqual(host.source.layers.map(item => item.id), [4, 5, 10]);
});

test('a leaf delete accepts an explicit empty descendant scope and keeps all siblings', async () => {
  const host = structuralFixture();
  await host.api.applyConfirmedStructurePlan(structuralPlan(host,
    { deletes: [{ ref: '4', expectedDescendantIds: [], allowAppearanceChange: true }] }), async value => value);
  assert.deepEqual(host.source.layers.map(item => item.id), [5, 10]);
  assert.deepEqual(host.document.layers.map(item => item.id), [2, 3, 6, 9]);
});

test('all structural edit kinds require explicit appearance authorization before history or mutation', async () => {
  const host = structuralFixture();
  for (const [kind, entry] of [
    ['moves', { ref: '4', parentRef: '6' }],
    ['ungroups', { ref: '3', expectedDescendantIds: ['4', '5', '10', '11'] }],
    ['deletes', { ref: '4', expectedDescendantIds: [] }]
  ]) {
    await assert.rejects(host.api.applyConfirmedStructurePlan(structuralPlan(host, { [kind]: [entry] }), async () => {}), /allowAppearanceChange/);
  }
  assert.equal(host.modalCount, 0);
  assert.deepEqual(host.operations, []);
});

test('automation structural deletion cannot bypass the explicit current confirmation and matching plan ID', async () => {
  const image = layer(2, 'comm_sp_0001');
  const panel = await panelHarness([image], { stored: sourceManifest([image]) });
  const api = panel.sandbox.__PSD2UI_DEV__;
  const plan = { version: 1, confirmationId: 'current-plan', preconditions: [{ layerId: '2' }],
    deletes: [{ ref: '2', expectedDescendantIds: [], allowAppearanceChange: true }] };
  const input = { expectedDocumentPath: panel.host.document.path, plan, confirmationId: plan.confirmationId };
  await assert.rejects(api.applyConfirmedStructurePlan(input), /当前用户确认标记/);
  await assert.rejects(api.applyConfirmedStructurePlan({ ...input, confirmationId: 'old-plan',
    confirmationText: 'APPLY_CONFIRMED_STRUCTURE_PLAN' }), /confirmationId 与调用确认不一致/);
  assert.deepEqual(panel.host.document.layers.map(item => item.id), [2]);
  assert.deepEqual(panel.host.history, []);
  assert.equal(panel.writes, 0);
});

function automationStructureInput(panel, edits) {
  const all = [];
  const visit = items => items.forEach(item => { all.push(item); visit(item.layers || []); });
  visit(panel.host.document.layers);
  all.forEach(item => {
    item.moveTo = async (parent, placement) => {
      item.parent.layers.splice(item.parent.layers.indexOf(item), 1);
      const index = placement.beforeId != null ? parent.layers.findIndex(child => child.id === placement.beforeId)
        : placement.afterId != null ? parent.layers.findIndex(child => child.id === placement.afterId) + 1 : 0;
      parent.layers.splice(index, 0, item); item.parent = parent;
    };
    item.ungroup = async () => {
      const parent = item.parent;
      parent.layers.splice(parent.layers.indexOf(item), 1, ...item.layers);
    ...
    item.delete = async () => { item.parent.layers.splice(item.parent.layers.indexOf(item), 1); };
  });
  const plan = structuralPlan({ allLayers: () => all }, edits);
  return { expectedDocumentPath: panel.host.document.path, plan, confirmationId: plan.confirmationId,
    confirmationText: 'APPLY_CONFIRMED_STRUCTURE_PLAN' };
}

function configuredButtonFixture() {
  const button = layer(2, '购买按钮', 'group');
  button.layers = [layer(3, 'comm_bt_0001'), layer(4, '购买', 'text')];
  const outside = layer(5, '其他内容', 'group'); outside.layers = [layer(6, 'comm_sp_0001')];
  const layers = [button, outside, layer(7, 'comm_sp_0002')];
  const manifest = Core.executeAuthoringCommand(sourceManifest(layers), { command: 'apply-structured-group',
    input: { layerId: '2', name: button.name, semantic: 'button', structure: { version: 1,
      roles: [{ name: 'background', layerId: '3' }, { name: 'label', layerId: '4' }] } }
  }, { actor: 'human-panel' }).manifest;
  return { layers, button, outside, manifest };
}

for (const [label, edits, expectedCode] of [
  ['delete background', { deletes: [{ ref: '3', expectedDescendantIds: [], allowAppearanceChange: true }] }, 'PSD2UI_STRUCTURE_ROLE_MISSING'],
  ['move background out of owner', { moves: [{ ref: '3', parentRef: '5', allowAppearanceChange: true }] }, 'PSD2UI_STRUCTURE_ROLE_MISSING'],
  ['ungroup configured owner', { ungroups: [{ ref: '2', expectedDescendantIds: ['3', '4'], allowAppearanceChange: true }] }, 'PSD2UI_STRUCTURE_OWNER_REMOVED']
]) test(`panel automation rolls back a semantic reference regression: ${label}`, async () => {
  const fixture = configuredButtonFixture();
  const panel = await panelHarness(fixture.layers, { stored: fixture.manifest });
  const before = JSON.stringify(panel.stored);
  await assert.rejects(panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel, edits)), error => {
    assert.equal(error.code, 'PSD2UI_STRUCTURE_EDIT_INVALID');
    assert.ok(error.issues.some(issue => issue.code === expectedCode));
    return true;
  });
  assert.deepEqual(panel.host.document.layers.map(item => item.id), [2, 5, 7]);
  assert.deepEqual(fixture.button.layers.map(item => item.id), [3, 4]);
  assert.deepEqual(fixture.outside.layers.map(item => item.id), [6]);
  assert.strictEqual(fixture.button.layers[0].parent, fixture.button);
  assert.equal(JSON.stringify(panel.stored), before);
  assert.equal(panel.writes, 0);
  assert.deepEqual(panel.host.history, [false]);
});

test('panel automation rejects moving a declared visual state out of its surviving owner', async () => {
  const normal = layer(3, '正常', 'group'); normal.layers = [layer(4, 'comm_sp_0001')];
  const selected = layer(5, '选中', 'group'); selected.layers = [layer(6, 'comm_sp_0002')];
  const owner = layer(2, '状态组件', 'group'); owner.layers = [normal, selected];
  const layers = [owner, layer(7, 'comm_sp_0003')];
  const stored = Core.executeAuthoringCommand(sourceManifest(layers), { command: 'set-visual-states', input: {
    layerId: '2', visualStates: { defaultState: 'normal', states: [{ name: 'normal', layerId: '3' }, { name: 'selected', layerId: '5' }] }
  } }, { actor: 'mcp' }).manifest;
  const panel = await panelHarness(layers, { stored });
  await assert.rejects(panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel,
    { moves: [{ ref: '3', parentRef: 'document-root', allowAppearanceChange: true }] })), error =>
    error.issues.some(issue => issue.code === 'PSD2UI_VISUAL_STATE_SCOPE_INVALID'));
  assert.deepEqual(panel.host.document.layers.map(item => item.id), [2, 7]);
  assert.deepEqual(owner.layers.map(item => item.id), [3, 5]);
  assert.strictEqual(normal.parent, owner);
  assert.deepEqual(panel.stored, stored);
  assert.equal(panel.writes, 0);
  assert.deepEqual(panel.host.history, [false]);
});

test('panel automation rejects a preview reference that Core would otherwise silently reconcile away', async () => {
  const template = layer(3, '模板', 'group'); template.layers = [layer(4, 'comm_sp_0001')];
  const preview = layer(5, '预览', 'group'); preview.layers = [layer(6, 'comm_sp_0002')];
  const owner = layer(2, '列表', 'group'); owner.layers = [template, preview];
  const stored = Core.executeAuthoringCommand(sourceManifest([owner]), { command: 'apply-structured-group', input: {
    layerId: '2', name: owner.name, semantic: 'list', structure: { version: 1,
      roles: [{ name: 'item-template', layerId: '3' }], previewLayerIds: ['5'], layoutSource: 'explicit',
      layout: { direction: 'vertical', itemWidth: 100, itemHeight: 40, spacing: 10 } }
  } }, { actor: 'human-panel' }).manifest;
  const panel = await panelHarness([owner], { stored });
  await assert.rejects(panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel,
    { deletes: [{ ref: '5', expectedDescendantIds: ['6'], allowAppearanceChange: true }] })), error =>
    error.issues.some(issue => issue.code === 'PSD2UI_STRUCTURE_PREVIEW_REMOVED'));
  assert.deepEqual(owner.layers.map(item => item.id), [3, 5]);
  assert.deepEqual(preview.layers.map(item => item.id), [6]);
  assert.deepEqual(panel.stored, stored);
  assert.equal(panel.writes, 0);
  assert.deepEqual(panel.host.history, [false]);
});

for (const existingError of [false, true]) test(`panel automation deletes an unreferenced layer without blocking existing errors: ${existingError}`, async () => {
  const fixture = configuredButtonFixture();
  if (existingError) fixture.button.layers.shift(); // An artist's earlier edit; the saved role is already stale.
  const panel = await panelHarness(fixture.layers, { stored: fixture.manifest });
  const result = await panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel,
    { deletes: [{ ref: '7', expectedDescendantIds: [], allowAppearanceChange: true }] }));
  assert.deepEqual(panel.host.document.layers.map(item => item.id), [2, 5]);
  assert.equal(panel.stored.nodes['7'], undefined);
  assert.deepEqual(panel.stored.nodes['2'].structure.roles, fixture.manifest.nodes['2'].structure.roles);
  assert.equal(result.diagnostics.some(issue => issue.code === 'PSD2UI_STRUCTURE_ROLE_MISSING'), existingError);
  assert.equal(panel.writes, 1);
  assert.deepEqual(panel.host.history, [true]);
});

test('panel automation permits deleting an entire component and its owned role nodes', async () => {
  const fixture = configuredButtonFixture();
  const panel = await panelHarness(fixture.layers, { stored: fixture.manifest });
  await panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel,
    { deletes: [{ ref: '2', expectedDescendantIds: ['3', '4'], allowAppearanceChange: true }] }));
  assert.deepEqual(panel.host.document.layers.map(item => item.id), [5, 7]);
  ['2', '3', '4'].forEach(id => assert.equal(panel.stored.nodes[id], undefined));
  assert.equal(panel.stored.nodes['6'].semantic, 'image');
  assert.equal(panel.writes, 1);
  assert.deepEqual(panel.host.history, [true]);
});

test('panel automation rejects moving a configured component outside a legacy export root', async () => {
  const fixture = configuredButtonFixture();
  const root = layer(8, '导出根', 'group'); root.layers = [fixture.button, fixture.outside];
  fixture.manifest.document.rootLayerId = '8';
  const panel = await panelHarness([root, fixture.layers[2]], { stored: fixture.manifest });
  await assert.rejects(panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel,
    { moves: [{ ref: '2', parentRef: 'document-root', allowAppearanceChange: true }] })), error =>
    error.issues.some(issue => issue.code === 'PSD2UI_STRUCTURE_OWNER_REMOVED'));
  assert.deepEqual(panel.host.document.layers.map(item => item.id), [8, 7]);
  assert.deepEqual(root.layers.map(item => item.id), [2, 5]);
  assert.strictEqual(fixture.button.parent, root);
  assert.deepEqual(panel.stored, fixture.manifest);
  assert.equal(panel.writes, 0);
  assert.deepEqual(panel.host.history, [false]);
});

test('deleting a complete nested component cannot break its surviving collection owner', async () => {
  const fixture = configuredButtonFixture();
  const list = layer(8, '按钮列表', 'group'); list.layers = [fixture.button];
  const layers = [list, fixture.outside, fixture.layers[2]];
  const host = photoshopHost(layers);
  const manifest = Core.prepareManifestForExport(fixture.manifest, host.api.createSnapshot('document-root')).manifest;
  const stored = Core.executeAuthoringCommand(manifest, { command: 'apply-structured-group', input: {
    layerId: '8', name: list.name, semantic: 'list', structure: { version: 1,
      roles: [{ name: 'item-template', layerId: '2' }], layoutSource: 'explicit',
      layout: { direction: 'vertical', itemWidth: 100, itemHeight: 40, spacing: 10 } }
  } }, { actor: 'human-panel' }).manifest;
  const panel = await panelHarness(layers, { stored });
  await assert.rejects(panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel,
    { deletes: [{ ref: '2', expectedDescendantIds: ['3', '4'], allowAppearanceChange: true }] })), error =>
    error.issues.some(issue => issue.nodeId === stored.nodes['8'].id && /^PSD2UI_STRUCTURE/.test(issue.code)));
  assert.deepEqual(list.layers.map(item => item.id), [2]);
  assert.deepEqual(fixture.button.layers.map(item => item.id), [3, 4]);
  assert.deepEqual(panel.stored, stored);
  assert.equal(panel.writes, 0);
  assert.deepEqual(panel.host.history, [false]);
...
test('an existing missing role does not hide another newly deleted role on the same owner', async () => {
  const fixture = configuredButtonFixture();
  fixture.button.layers.pop();
  const panel = await panelHarness(fixture.layers, { stored: fixture.manifest });
  await assert.rejects(panel.sandbox.__PSD2UI_DEV__.applyConfirmedStructurePlan(automationStructureInput(panel,
    { deletes: [{ ref: '3', expectedDescendantIds: [], allowAppearanceChange: true }] })), error =>
    error.issues.some(issue => issue.nodeId === fixture.manifest.nodes['2'].id));
  assert.deepEqual(fixture.button.layers.map(item => item.id), [3]);
  assert.deepEqual(panel.stored, fixture.manifest);
  assert.equal(panel.writes, 0);
  assert.deepEqual(panel.host.history, [false]);
});
