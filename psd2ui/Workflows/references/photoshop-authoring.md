...
先读 [PS-MCP 环境与最小示例](../../PS-MCP/README.md)。当前是 CEP 9 分支，默认连接已经打开的 CEP 面板。Windows 自带 PowerShell 5.1 即可，Node.js 仅供外部 MCP 使用。
...
本分支已有 moves/ungroups/deletes，必须明确 allowAppearanceChange:true；解组/删除还要提供 expectedDescendantIds（叶子为空数组）。成组只允许连续同级成员，保持完整剪贴链。能力与约束以本分支宿主为准。
...
CEP 写操作超时后用原 requestId/instanceId 查询 status 回执，不自动重放。旧 UXP 会话恢复也必须重新按本分支入口 Check。
