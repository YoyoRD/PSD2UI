...
function createPsd2UiMcpServer(host, options = {}) {
  ...
  server.registerTool('psd2ui_photoshop_status', {
    ...
    description: '读取 Photoshop PSD2UI 状态。workflow 启动前传 checkOnly:true：只检查已有服务、插件会话与分析 API，异常直接报错，不启动服务、不加载或重载插件；无需打开 PSD。省略时保留自动准备连接的行为。',
    inputSchema: z.object({ checkOnly: z.boolean().optional() }),
  ...
  server.registerTool('psd2ui_apply_confirmed_structure_plan', {
    ...
    description: '按精确 PSD 路径、图层前置条件 preconditions 与 confirmationId 原子应用计划；支持 containers 普通组、copies、renames、groups 组件合组、adopt 已有组和 presets。containers 使用 alias/name/members，返回真实组 ID，可随后设置视觉状态。不会开放 module、Common 或资源迁移权限。',
    inputSchema: z.object({
      expectedDocumentPath: z.string().min(1),
