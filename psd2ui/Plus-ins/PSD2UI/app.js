...
function selectionKey() {
  try { return `${documentKey()}|${getActiveLayersInfo().map((layer) => layer.id).join(',')}`; }
  catch (error) { return ''; }
}
...
async function refreshAuthoringState() {
  ...
  try {
    ...
    const revision = sidecar.manifest ? ` · revision ${sidecar.manifest.revision || 0}` : ' · 尚未生成';
    element('sidecar-path').textContent = `${sidecar.path}${revision}`;
  } catch (error) {
  ...
  if (selected.length === 1) {
    ...
    const changes = Psd2Ui.diffLayerFromBaseline(manifest, readLayer(layer.layer));
    ...
    const changed = selected.map((layer) => ({
      name: layer.name,
      changes: Psd2Ui.diffLayerFromBaseline(manifest, readLayer(layer.layer))
...
async function applyConfirmedStructurePlanForAutomation(options) {
  ...
  return runDocumentMutationWithManifestRollback(current, (persistInMutation) =>
    applyConfirmedStructurePlan(plan, async (applied) => {
      const snapshot = createSnapshot(current.document.rootLayerId);
      let prepared = Psd2Ui.prepareManifestForExport(current, snapshot, {
