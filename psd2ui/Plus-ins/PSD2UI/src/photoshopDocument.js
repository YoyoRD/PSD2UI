...
function readLayer(layer) {
  ...
  const descriptor = isText ? readFullLayerDescriptor(layer) : {};
  const effectsDescriptor = descriptor.layerEffects || null;
  ...
  let bounds = readBounds(layer);
  if (text && text.layoutMode === 'point' && layer.boundsNoEffects) {
    ...
    if (Object.values(candidate).every(Number.isFinite)) bounds = candidate;
  }
...
function validateGroupSelection(selected) {
  const document = requireDocument();
...
function assertBounds(layer, expected, label) {
  if (!expected) return;
  const actual = readBounds(layer);
  ['left', 'top', 'right', 'bottom'].forEach((key) => {
    const target = Number(expected[key]);
    if (!Number.isFinite(target) || Math.abs(actual[key] - target) > 1) {
      throw new Error(
...
function assertLayerPrecondition(document, condition) {
  ...
  if (condition.parentId != null) {
    ...
    if (actualParentId !== String(condition.parentId)) {
      throw new Error(
  ...
  assertBounds(layer, condition.bounds, `图层 ${layerId}`);
  return layer;
}
...
function isDescendantOrSelf(root, layer) {
  let current = layer;
  while (current) {
    if (String(current.id) === String(root.id)) return true;
  ...
  return core.executeAsModal(async (executionContext) => {
    ...
    try {
      ...
      const resolveRef = (rawRef) => {
        ...
        const layer = isAliasRef(ref)
          ? aliases.get(ref)
          : findLayerById(document.layers || [], ref);
