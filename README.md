...
先完成上面的面板安装。AI 所在电脑需要 Node.js（20.x 至少 20.19，22.x 至少 22.12，或工具支持的更新版本）。本分支需要 PowerShell 7、Adobe UDT 和已经加载的 UXP 插件。在仓库根执行：
...
Check 成功后再继续。此工具链运行在 Photoshop 所在电脑上；UDT 会话需对应当前工具包。
...
- 本 CCX 分支没有 CEP 的 moves/ungroups/deletes 扩展，相关层级修改需要人工承接。
