...
本 CCX 分支没有通用 moves/ungroups/deletes 接口，也没有任意 move/reparent/reorder/ungroup/delete 工具。不要提交 CEP 专用字段或把另一个分支能力当作本地已支持；需要此类层级修改时明确交由人工承接。全树快照会把顶层 parentId 显示为 document-root，但本分支的结构前置条件仍比较原始 parentId。计划涉及顶层层时，以该层真实 ID 再取一次局部 snapshot，使用返回的 root.parentId（可能为空字符串），不能直接把虚拟根名填入前置条件；这属于补齐操作输入。
